...
//
// This relies on parsing the leading number out of the message text (e.g.
// "3 people answered your question.") since the notifications table has no
// dedicated count column. If no number is found, it's treated as count = 1.
export const groupNewAnswerNotification = async ({
  ...
  const existingSql = `
    ...
    LIMIT 1
  `;
  // Match any existing notification whose link points at this same question,
  // regardless of which answer it currently points to.
  const likePattern = `/question/${questionHash}#answer-%`;
  const existingRows = await safeExecute(existingSql, [userId, likePattern]);

  if (existingRows.length === 0) {
    // First answer notification for this question — create a fresh row.
...
export const notifyAnswerSeen = async ({
  ...
  if (answererId === viewerId) {
    return null; // don't notify someone that they "saw" their own answer
  }
  ...
  if (existing.length > 0) {
    return null; // already notified for this answer being seen
  }
