...
export default function QuestionDetail() {
  ...
  // Notify each answer's author that the asker has seen their answer.
  useEffect(() => {
    if (!isOwnQuestion || answers.length === 0) return;
    answers.forEach((answer) => {
      notificationService.markAnswerSeen(answer.id).catch(() => {});
    });
  }, [answers, isOwnQuestion]);
  ...
  function handleInsert([before, after]) {
    ...
    const next =
      answerText.slice(0, s) +
      before +
      answerText.slice(s, e) +
      after +
      answerText.slice(e);
    setAnswerText(next);
  ...
  async function handleCheckFit() {
    ...
    try {
      ...
      const errorMsg =
        err?.response?.data?.msg || "Could not evaluate fit. Please try again.";
  ...
  const authorName =
    question.author?.username || question.author?.name || "New User";
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {answers.length === 0 ? (
          ...
          >
            <div style={{ fontSize: 32, marginBottom: 10, color: "#ccc" }}>
              💬
            </div>
        ...
        {isOwnQuestion ? (
          ...
          >
            ...
            {postError && (
              <p style={{ color: "#e74c3c", fontSize: 14, marginBottom: 10 }}>
                {postError}
              </p>
            )}
            ...
            {fitResult && (
              <FitPanel level={fitResult.level} note={fitResult.note} />
            )}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <button
                  ...
                  style={{
                    ...
                    cursor:
                      answerText.trim().length < 20 ? "not-allowed" : "pointer",
                  }}
                >
